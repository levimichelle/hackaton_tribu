...
def find_place(received_location, received_hobby):
...
def return_place_info(received_location, received_hobby):
    place = find_place(received_location, received_hobby)
    return consts.DICT_PLACES_VOLUNTEER[place][consts.INFO_INDEX]


def show_volunteering(received_location, received_hobby):
    app = tkinter.Tk()
    app.title("Volunteer Quiz")
    app.geometry("350x350+500+100")
    app.configure(bg="lightblue")
    place_name = find_place(received_location, received_hobby)
    place_info = return_place_info(received_location, received_hobby)
...
# show_volunteering(received_location, received_hobby)
